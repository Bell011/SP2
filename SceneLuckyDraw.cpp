...
void SceneLuckyDraw::Init()
{
	...
	//meshList[GEO_ARROW]->textureID = LoadTGA("Image//whe.tga");
...
void SceneLuckyDraw::RenderPrizes()
{
	...
	if (bSpinAnimation)
	{
		if (fAnimationDuration > 5.0)
		{
			...
			RenderTextOnScreen(meshList[GEO_TEXT], "YOU WON " + prizes[iRNG], Color(1, 1, 0), 3, 0, 0);

		}
		else
			RenderTextOnScreen(meshList[GEO_TEXT], "Spinning", Color(1, 1, 1), 3, 0, 0);
	}
	else
	{
		RenderTextOnScreen(meshList[GEO_TEXT], "Press space to spin wheel", Color(0, 1, 0), 3, 0, 0);
	}
}
